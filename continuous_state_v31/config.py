...
class ContinuousStateV31Config:
    """Fixed, pre-registered parameters for the v3.1 experiment."""

    output_dir: str = "outputs_continuous_state_v31"
    ...
    plateau_candidate_valid_cycles: int = 300
    plateau_lock_valid_cycles: int = 500
    plateau_failure_reset_valid_cycles: int = 150
    plateau_reference_valid_cycles: int = 500
    exit_candidate_valid_cycles: int = 300
    exit_confirm_valid_cycles: int = 500
    exit_failure_reset_valid_cycles: int = 150
    source_plateau_threshold_quantile: float = .75
    source_plateau_threshold_sensitivity: tuple[float, ...] = (.50, .60, .75)
    ...
    rolling_metric_observations: int = 500
    rolling_metric_export_stride: int = 25
    ...
    def jsonable(self) -> dict[str, Any]:
        data = asdict(self)
        for key in ("pair_gap_bins", "forecast_horizons_cycles", "source_plateau_threshold_sensitivity"):
            data[key] = [list(value) if isinstance(value, tuple) else value for value in data[key]]
        data["features"] = list(FEATURES)
        data["code_version"] = "csv31"
        return data
