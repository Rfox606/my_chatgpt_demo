...
from .state_engine import EVIDENCE_NAMES, SourceSupport, main_state_columns, run_target_state
...
def prefix_causality(
    ...
    for cutoff in (2000, 5000, 10000):
        ...
        if prefix.empty:
            ...
            continue
        scored, _, _ = run_target_state(prefix, protocol_id, features, config, source_support)
...
def no_future_check(states: pd.DataFrame, config: ContinuousStateV43Config) -> dict[str, object]:
    frozen = bool(not states.empty and (states.start_cycle.to_numpy(float) > config.baseline_cycles).all() and
                  states.baseline_frozen.eq(1).all())
    no_stage = not bool(FORBIDDEN_COLUMNS.intersection(states.columns))
    return {"status": "PASS" if frozen and no_stage else "FAIL", "calibration_rows_not_emitted": frozen,
