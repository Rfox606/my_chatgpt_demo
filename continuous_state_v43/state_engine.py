...
from .config import FEATURE_GROUPS, ContinuousStateV43Config
...
def _online_residual_score(values: dict[str, np.ndarray], refs: dict[str, GroupReference], frame: pd.DataFrame, config: ContinuousStateV43Config) -> np.ndarray:
...
def _run_evidence_tracks(
    ...
    config: ContinuousStateV43Config,
    protocol_id: str,
    time_basis: str,
) -> tuple[dict[str, np.ndarray], pd.DataFrame, np.ndarray]:
    n = len(frame)
    coordinate = "center_cycle_actual" if time_basis == "actual" else "center_cycle_effective"
    cycles = frame[coordinate].to_numpy(float)
    increments = np.diff(cycles, prepend=cycles[0])
    if n > 1:
        increments[0] = float(np.median(np.diff(cycles)));
    increments = np.maximum(increments, 0.0)
    guard = frame.is_restart_guard.to_numpy(bool)
    values: dict[str, np.ndarray] = {"evidence_increment_cycles": np.where(guard, 0.0, increments)}
    ...
    for index in range(n):
        ...
        for name in EVIDENCE_NAMES:
            ...
            if conditions[name][index]:
                runs[name] += increments[index]; false_runs[name] = 0.0
            else:
                false_runs[name] += increments[index]
...
def run_target_state(
    ...
    if time_basis not in {"effective", "actual"}:
        raise ValueError("time_basis must be effective or actual")
    ...
    cycles = ordered[f"center_cycle_{time_basis}"].to_numpy(float)
    ...
    evidence, events, cusum = _run_evidence_tracks(monitor_frame, conditions, residual[monitor], thresholds, continuous, config, protocol_id, time_basis)
    ...
    output: dict[str, object] = {
        ...
        "start_cycle": monitor_frame.start_cycle_effective.to_numpy(float), "end_cycle": monitor_frame.end_cycle_effective.to_numpy(float), "center_cycle": monitor_frame.center_cycle_effective.to_numpy(float),
        "start_cycle_effective": monitor_frame.start_cycle_effective.to_numpy(float), "end_cycle_effective": monitor_frame.end_cycle_effective.to_numpy(float), "center_cycle_effective": monitor_frame.center_cycle_effective.to_numpy(float),
        "start_cycle_actual": monitor_frame.start_cycle_actual.to_numpy(float), "end_cycle_actual": monitor_frame.end_cycle_actual.to_numpy(float), "center_cycle_actual": monitor_frame.center_cycle_actual.to_numpy(float),
        "cycle_effective": monitor_frame.cycle_effective.to_numpy(float), "cycle_actual": monitor_frame.cycle_actual.to_numpy(float), "state_time_basis": np.full(len(monitor_frame), time_basis),
        ...
        "baseline_cycles": np.full(len(monitor_frame), config.baseline_cycles), "monitoring_start_cycle_effective": np.full(len(monitor_frame), float(monitor_frame.start_cycle_effective.min()) if len(monitor_frame) else np.nan),
