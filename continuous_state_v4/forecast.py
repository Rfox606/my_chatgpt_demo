...
TARGETS = ("D_state", "V50_norm", "A_state", "abrupt_score")
INPUTS = (
    "D_state", "V20_norm", "V50_norm", "V100_norm", "direction_consistency",
    "A_state", "state_volatility", "weighted_oos", "abrupt_score", "abrupt_cusum",
)
...
def train_frozen_models(source_states: pd.DataFrame, config: ContinuousStateV4Config) -> dict[tuple[str, int], Ridge | None]:
        ...
        for target in TARGETS:
            ...
            for index in range(len(frame)):
                ...
                if index % issue_step == 0 and not bool(frame.is_restart_guard.iloc[index]) and vectors[index] is not None and np.isfinite(response[index]):
                    ...
                    frozen_delta = float(model.predict(vector.reshape(1, -1))[0]) if model is not None else 0.0
                ...
                # Kalman receives the current observation only after issuing this cycle's forecast.
