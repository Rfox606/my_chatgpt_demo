...
def make_figures(states: pd.DataFrame, events: pd.DataFrame, ablations: pd.DataFrame, metrics: pd.DataFrame, output) -> None:
    ...
    for dataset, frame in targets.items():
        ...
        axes[1].plot(frame.center_cycle, frame.V20_norm, linewidth=.7, label=f"{dataset}: V20")
        axes[1].plot(frame.center_cycle, frame.V100_norm, linewidth=.7, linestyle="--", label=f"{dataset}: V100")
    ...
    _save(output / "fig_v4_state_trajectories.png")

    fig, axes = plt.subplots(3, 1, figsize=(12, 9), sharex=True)
    for dataset, frame in targets.items():
        for group in ("rs", "rx", "ry"):
            axes[0].plot(frame.center_cycle, frame[f"velocity_v20_{group}_contribution"], linewidth=.6, label=f"{dataset}:{group}")
            axes[1].plot(frame.center_cycle, frame[f"velocity_v50_{group}_contribution"], linewidth=.6, label=f"{dataset}:{group}")
            axes[2].plot(frame.center_cycle, frame[f"velocity_v100_{group}_contribution"], linewidth=.6, label=f"{dataset}:{group}")
    for axis, label in zip(axes, ("V20 channel contribution", "V50 channel contribution", "V100 channel contribution")):
        axis.set_ylabel(label)
        axis.set_ylim(-.02, 1.02)
        axis.legend(fontsize=7, ncol=3)
    axes[-1].set_xlabel("Cycle")
    _save(output / "fig_v4_velocity_channel_contributions.png")
    ...
    axes[-1].set_xlabel("Cycle")
    _save(output / "fig_v4_evidence_tracks.png")
    ...
    if not ablations.empty:
        ...
        plt.scatter(np.arange(len(values)), values.trajectory_spearman, s=12, c=values.event_position_stable.map({True: "tab:blue", False: "tab:red"}))
    ...
    _save(output / "fig_v4_ablation_stability.png")
    ...
    _save(output / "fig_v4_forecast_comparison.png")
