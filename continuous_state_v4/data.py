...
from .config import ContinuousStateV4Config, FEATURES
...
def assert_label_free(frame: pd.DataFrame) -> None:
    ...
    if leaked:
        raise AssertionError(f"Forbidden stage column(s) reached a v4 model boundary: {leaked}")


def load_window_table(config: ContinuousStateV4Config) -> pd.DataFrame:
...
def add_restart_guard(frame: pd.DataFrame, config: ContinuousStateV4Config) -> pd.DataFrame:
...
def baseline_mask(frame: pd.DataFrame, config: ContinuousStateV4Config) -> np.ndarray:
    assert_label_free(frame)
