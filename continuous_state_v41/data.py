...
from .config import ContinuousStateV41Config, FEATURES
...
def assert_label_free(frame: pd.DataFrame) -> None:
    ...
    if leaked:
        raise AssertionError(f"Forbidden stage column(s) reached a v4.1 model boundary: {leaked}")


def load_window_table(config: ContinuousStateV41Config) -> pd.DataFrame:
...
def add_restart_guard(frame: pd.DataFrame, config: ContinuousStateV41Config) -> pd.DataFrame:
...
def baseline_mask(frame: pd.DataFrame, config: ContinuousStateV41Config) -> np.ndarray:
    assert_label_free(frame)
